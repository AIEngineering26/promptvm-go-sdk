...
func TestMarketplaceCreatorClaimMarketplaceCreatorProfileWithWireMock(
	t *testing.T,
) {
	wiremockPort := os.Getenv("WIREMOCK_PORT")
	if wiremockPort == "" {
	...
	_, invocationErr := client.MarketplaceCreator.ClaimMarketplaceCreatorProfile(
		context.TODO(),
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestMarketplaceCreatorClaimMarketplaceCreatorProfileWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestMarketplaceCreatorClaimMarketplaceCreatorProfileWithWireMock", "POST", "/api/v1/marketplace/creator/creatorUserId/claim", nil, 1)
}
