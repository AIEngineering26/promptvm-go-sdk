...
package prompt_resolution_test
...
func TestPromptResolutionResolvePromptWithWireMock(
	t *testing.T,
) {
	wiremockPort := os.Getenv("WIREMOCK_PORT")
	if wiremockPort == "" {
	...
	_, invocationErr := client.PromptResolution.ResolvePrompt(
		...
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestPromptResolutionResolvePromptWithWireMock"}},
		),
	)
...
func TestPromptResolutionResolvePromptStreamWithWireMock(
	...
	invocationErr := client.PromptResolution.ResolvePromptStream(
		...
		request,
		option.WithHTTPHeader(
			http.Header{"X-Test-Id": []string{"TestPromptResolutionResolvePromptStreamWithWireMock"}},
		),
	)

	require.NoError(t, invocationErr, "Client method call should succeed")
	VerifyRequestCount(t, "TestPromptResolutionResolvePromptStreamWithWireMock", "GET", "/api/v1/prompts/promptId/resolve/stream", nil, 1)
}
